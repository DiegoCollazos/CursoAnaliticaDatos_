...
\begin{enumerate}[leftmargin=1.2cm]
  ...
  \item Agrega un widget \textbf{Preprocess} y conéctale la salida \textit{Data Sample} (entrenamiento). Configúralo con \textbf{Impute Missing Values}: \texttt{Age} $\to$ \textit{Median}, y las variables categóricas (incluida \texttt{Embarked}) $\to$ \textit{Most frequent value}. Este widget tiene dos salidas: \textbf{Preprocessed Data} (el entrenamiento ya tratado) y \textbf{Preprocessor} (la ``receta'' aprendida: la mediana de \texttt{Age} y la moda de \texttt{Embarked}, calculadas \textbf{solo} con el entrenamiento) --- por ahora solo usaremos la primera.
  \item Agrega un widget \textbf{Apply Domain} (categoría \textit{Data}). Conéctale la salida \textit{Remaining Data} (prueba) de \textit{Data Sampler} como \textbf{Data}, y la salida \textit{Preprocessed Data} del \textit{Preprocess} (entrenamiento) como \textbf{Reference Data}. \textit{Apply Domain} \textbf{no vuelve a calcular nada}: reconstruye el conjunto de prueba usando exactamente el mismo dominio que quedó fijado al preprocesar el entrenamiento (la misma mediana de \texttt{Age} y la misma moda de \texttt{Embarked}), sin recalcular esos valores con los datos de prueba.
\end{enumerate}

\begin{nota}
\textit{Preprocess} (entrenamiento) $\to$ \textit{Apply Domain} (con la prueba como \textit{Data} y el entrenamiento ya preprocesado como \textit{Reference Data}) es el equivalente visual exacto de ``ajustar (\textit{fit}) en entrenamiento y transformar (\textit{transform}) en prueba con los mismos parámetros'', que en el notebook se resuelve calculando \texttt{mediana\_age} y \texttt{moda\_embarked} solo con \texttt{train} y aplicándolas también a \texttt{test}. (El widget \textit{Preprocess} solo acepta \textit{Data} como entrada --- no se puede encadenar su salida \textit{Preprocessor} a un segundo \textit{Preprocess}; para aplicar una transformación ya aprendida a otro conjunto de datos, el widget correcto en Orange es \textit{Apply Domain}.)
\end{nota}

\begin{actividad}
Conecta un widget \textbf{Data Table} a la salida \textit{Preprocessed Data} del \textit{Preprocess} (entrenamiento) y a la salida \textit{Data} del \textit{Apply Domain} (prueba), y verifica que ninguna de las dos columnas tratadas (\texttt{Age}, \texttt{Embarked}) tenga valores faltantes. En un cuadro de texto junto al \textit{Apply Domain}, anota el valor con el que quedó imputada \texttt{Age} y compáralo con la mediana que calculó el notebook ($\approx$ 28.5 años). Incluye en tu PDF de evidencias una captura del workflow completo de esta sección (\textit{Select Columns} $\to$ \textit{Data Sampler} $\to$ \textit{Preprocess} y \textit{Apply Domain} $\to$ las dos \textit{Data Table}).
\end{actividad}
