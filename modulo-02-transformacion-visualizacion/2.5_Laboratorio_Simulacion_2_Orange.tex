...
\begin{enumerate}[leftmargin=1.2cm]
  ...
  \item Agrega un segundo widget \textbf{Feature Statistics} y conéctale la salida \textit{Data Sample} (entrenamiento) --- para leer la mediana de \texttt{Age} calculada \textbf{solo} con el entrenamiento (no la del dataset completo que viste en 1.1). Anota ese valor: lo necesitarás en el siguiente paso.
  \item Agrega el widget \textbf{Impute} (categoría \textit{Data}, distinto del paso ``Impute Missing Values'' dentro de \textit{Preprocess}) y conéctale también la salida \textit{Data Sample} (entrenamiento). A diferencia de \textit{Preprocess}, \textit{Impute} sí permite elegir un tratamiento distinto \textbf{para cada variable}:
  \begin{itemize}
    \item Selecciona \texttt{Age} en la lista de variables y cambia su tratamiento individual a \textbf{Value} (valor fijo), escribiendo el valor de la mediana que anotaste en el paso anterior. Como Orange no trae un preset listo de ``mediana'', así se logra el mismo efecto: imputar con un valor fijo igual a la mediana del entrenamiento.
    \item Selecciona \texttt{Embarked} y déjalo (o cámbialo) en \textbf{Average/Most frequent} --- para una variable categórica, esta opción imputa con la moda.
    \item El nombre y la ubicación exactos de estas opciones pueden variar un poco según la versión de Orange; si no las encuentras igual, explora el panel del widget o pregúntale al docente.
  \end{itemize}
  \item Agrega un widget \textbf{Apply Domain} (categoría \textit{Data}). Conéctale la salida \textit{Remaining Data} (prueba) de \textit{Data Sampler} como \textbf{Data}, y la salida del \textit{Impute} (entrenamiento) como \textbf{Reference Data}. \textit{Apply Domain} \textbf{no vuelve a calcular nada}: reconstruye el conjunto de prueba usando exactamente el mismo dominio que quedó fijado al imputar el entrenamiento (el mismo valor fijo de \texttt{Age} y la misma moda de \texttt{Embarked}), sin recalcular esos valores con los datos de prueba.
\end{enumerate}

\begin{nota}
\textit{Impute} (entrenamiento) $\to$ \textit{Apply Domain} (con la prueba como \textit{Data} y el entrenamiento ya imputado como \textit{Reference Data}) es el equivalente visual exacto de ``ajustar (\textit{fit}) en entrenamiento y transformar (\textit{transform}) en prueba con los mismos parámetros'', que en el notebook se resuelve calculando \texttt{mediana\_age} y \texttt{moda\_embarked} solo con \texttt{train} y aplicándolas también a \texttt{test}. Usamos el widget \textit{Impute} en vez del paso ``Impute Missing Values'' de \textit{Preprocess} porque este último solo ofrece tres opciones \textbf{globales} (Average/Most frequent, Replace with random value, Remove rows) --- no deja elegir un método distinto por variable, y ``Average'' calcula la \textbf{media}, no la mediana. Tampoco se puede encadenar la salida \textit{Preprocessor} de un \textit{Preprocess} a otro \textit{Preprocess}: ese widget solo acepta \textit{Data} como entrada; para aplicar una transformación ya aprendida a otro conjunto de datos, el widget correcto en Orange es \textit{Apply Domain}.
\end{nota}

\begin{actividad}
Conecta un widget \textbf{Data Table} a la salida del \textit{Impute} (entrenamiento) y a la salida \textit{Data} del \textit{Apply Domain} (prueba), y verifica que ninguna de las dos columnas tratadas (\texttt{Age}, \texttt{Embarked}) tenga valores faltantes. En un cuadro de texto junto al \textit{Impute}, anota el valor fijo que usaste para \texttt{Age} y compáralo con la mediana que calculó el notebook ($\approx$ 28.5 años). Incluye en tu PDF de evidencias una captura del workflow completo de esta sección (\textit{Select Columns} $\to$ \textit{Data Sampler} $\to$ \textit{Feature Statistics}, \textit{Impute} y \textit{Apply Domain} $\to$ las dos \textit{Data Table}).
\end{actividad}

\subsection{1.3 Discretización de la edad y codificación de categóricas}

\begin{enumerate}[leftmargin=1.2cm]
  \item Agrega el widget \textbf{Discretize} (categoría \textit{Data}) conectado a la salida del \textit{Impute} (entrenamiento). Selecciona la variable \texttt{Age} y prueba, uno a la vez, los tres métodos de discretización disponibles:
  \begin{itemize}
