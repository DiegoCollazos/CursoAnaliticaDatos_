...
{\large \textbf{Docente:} Diego Fabián Collazos Huertas}\\[1.5cm]

\begin{tabular}{|p{4cm}|p{9cm}|}
\hline
\textbf{Nombre del estudiante} & \\[0.6cm]\hline
\textbf{Documento} & \\[0.6cm]\hline
\textbf{Grupo / equipo} & \\[0.6cm]\hline
\textbf{Fecha de entrega} & \\[0.6cm]\hline
\end{tabular}
...
\begin{enumerate}[leftmargin=1.2cm]
  \item Trabaja individualmente o en el equipo asignado por el docente.
  \item Para cada parte del laboratorio, arma el flujo (\textit{workflow}) solicitado en el lienzo de Orange y responde las actividades marcadas con el ícono \faPencil.
  \item El informe final debe incluir, para cada parte: \textbf{una captura de pantalla del workflow completo} armado en Orange, y las respuestas escritas a cada actividad.
...
\begin{actividad}
Compara el histograma/gráfico de \texttt{quality} que obtuviste en Orange con el resultado de \texttt{vinos['quality'].value\_counts()} del notebook. ¿Coinciden los conteos por nivel de calidad? Adjunta una captura del widget \textit{Distributions}.
\end{actividad}
...
\begin{actividad}
Compara el \textit{Scatter Plot} de Orange con el gráfico equivalente hecho con Matplotlib en el notebook (potencia vs. eficiencia, coloreado por cilindros). ¿Se observa la misma tendencia (a mayor potencia, menor eficiencia)? Adjunta una captura del \textit{Scatter Plot}.
\end{actividad}
...
ORDER BY quality;
\end{lstlisting}
Conecta el resultado a un widget \textbf{Bar Chart} (o \textbf{Distributions}) mostrando \texttt{num\_vinos} por \texttt{quality}. ¿El resultado coincide con la tabla \texttt{resumen\_sql} del notebook? Adjunta una captura del workflow completo (SQL Table $\to$ Data Table $\to$ Bar Chart).
\end{actividad}
...
\begin{actividad}
Abre la vista previa de columnas del widget \textit{File} y responde:
...
\end{enumerate}
Adjunta una captura de la vista previa de columnas del widget \textit{File}, mostrando el ajuste que hiciste.
\end{actividad}
...
\begin{actividad}
Compara ambos gráficos con los del notebook (barras de género musical, histograma de tempo). ¿El género más frecuente coincide? ¿La distribución del tempo tiene una forma parecida? Adjunta dos capturas del widget \textit{Distributions}: una con \texttt{track\_genre} seleccionada y otra con \texttt{tempo}.
\end{actividad}
...
Sube al repositorio del curso (o entrega por el medio que indique el docente) un único documento (PDF o Word) que contenga:

\begin{enumerate}[leftmargin=1.2cm]
  \item Los datos de la portada de este documento diligenciados.
  \item Para cada una de las 5 actividades (\faPencil): la captura de pantalla solicitada y la respuesta escrita.
  \item Una breve reflexión final (5-8 líneas): ¿en qué escenarios preferirías usar Orange sobre Python, y en cuáles preferirías Python sobre Orange, a partir de lo que experimentaste en este laboratorio?
\end{enumerate}
